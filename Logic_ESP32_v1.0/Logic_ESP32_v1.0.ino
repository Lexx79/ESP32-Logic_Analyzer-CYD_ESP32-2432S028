...
// ИСПРАВЛЕНО: Правильная логика для масштабирования в обе стороны
const float display_divisors[] = {0.125, 0.25, 0.5, 1.0, 2.0, 4.0, 8.0}; // 7 уровней
int current_display_idx = 3; // Индекс для делителя 1.0 (нативный масштаб)
...
void loop() {
  ...
  if (!hold) {
    if (current_trig_mode == T_AUTO) {
      readData(320);
      view_offset = 0;
      current_display_idx = 3; // В авто-режиме масштаб всегда x1
      drawData();
    } else { // FALL или RISE
        if(waitForTrigger(100000)) { // 100ms timeout
            readData(320);
            view_offset = 0;
...
bool waitForTrigger(unsigned long timeout_us) {
  ...
  while (micros() - startTime < timeout_us) {
    bool state = digitalRead(CHANNEL_1_PIN);
    delayMicroseconds(1);
    bool nextState = digitalRead(CHANNEL_1_PIN);
    if (effective_trig == T_RISE && !state && nextState) return true;
    if (effective_trig == T_FALL && state && !nextState) return true;
  }
  return false;
}
...
void readData(int samples) {
  ...
  int start_idx = (current_trig_mode == T_AUTO) ? WAVE_START_X : 0;
...
void drawData() {
  ...
  float pixels_per_sample = display_divisors[current_display_idx];

  for (int x = WAVE_START_X; x < 320; x++) {
    int data_idx = view_offset + (int)((x - WAVE_START_X) / pixels_per_sample);
    int data_idx_prev = view_offset + (int)((x - 1 - WAVE_START_X) / pixels_per_sample);
    ...
    if (pixels_per_sample >= 1) { // Растягиваем (Zoom In)
...
void updateUI() {
  ...
  if (current_trig_mode == T_SINGLE) {
    tft.drawRoundRect(BTN_X1, MAIN_BTN_Y, BTN_W, BTN_H, 5, TFT_GREEN);
    tft.setTextColor(TFT_GREEN);
    tft.drawString("ARM", BTN_X1 + BTN_W / 2, MAIN_BTN_Y + BTN_H / 2);
  } else {
    tft.drawRoundRect(BTN_X1, MAIN_BTN_Y, BTN_W, BTN_H, 5, TFT_DARKGREY);
  }
...
void checkTouch() {
  ...
  if (tft.getTouch(&x, &y) && (millis() - last_touch_time > 200)) {
    ...
    if (y > MAIN_BTN_Y && x > BTN_X4 && x < BTN_X4 + BTN_W) { 
    ...
    else if (current_trig_mode == T_SINGLE && hold && y > MAIN_BTN_Y && x > BTN_X1 && x < BTN_X1 + BTN_W) {
        tft.setTextDatum(TC_DATUM);
        tft.setTextSize(1);
        tft.setTextColor(TFT_RED);
        tft.drawString("WAITING...", 160, 5);
        delay(10);

        if (waitForTrigger(3000000)) {
            readData(CAPTURE_BUFFER_SIZE);
            view_offset = 0;
            current_display_idx = 3; // Сброс на 1.0
       ...
       if (x > BTN_X3 && x < BTN_X3 + BTN_W) { // T+
        ...
        float divisor = display_divisors[current_display_idx];
        int shift_amount = (int)((320.0f / divisor) / 4.0f);
